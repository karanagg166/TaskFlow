...
const GroupPage: React.FC = () => {
    ...
    const fetchTasks = async () => {
        ...
        try {
            const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/${groupId}/${id}/tasks`, {
    ...
    return (
        <div className="container mx-auto min-h-screen flex flex-col justify-center" style={{ paddingTop: '80px', paddingBottom: '80px' }}>
            <Typography variant="h4" className="font-bold text-blue-800 mb-4 text-center">Tasks for Group</Typography>
                    ...
                    >
                        <Card className={`bg-gradient-to-r ${getPriorityColor(task.priority || 'Low')} p-6 text-white transition-transform duration-200 ease-in-out hover:scale-105`}>
                            <CardContent>
                                ...
                                <Typography className="mt-3 font-bold text-lg">{`Status: ${task.status || 'Pending'}`}</Typography>
